      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <span
              className={`text-sm font-semibold ${isOnline ? "text-green-600" : "text-gray-500"}`}
            >
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="text-sm">
            ...
            <p className="text-xs text-gray-400 mt-1">
              {ambulance.location.latitude.toFixed(4)},{" "}
              {ambulance.location.longitude.toFixed(4)}
            </p>
          </div>
        </div>
...
const EmptyState = ({ isOnline }) => {
  return (
    <div className="bg-white rounded-lg shadow-md p-12 text-center">
      <div
        className={`inline-flex p-4 rounded-full mb-4 ${isOnline ? "bg-blue-100" : "bg-gray-100"}`}
      >
        {isOnline ? (
...
const ActiveCaseView = ({ request, ambulanceLocation, onComplete }) => {
  ...
  useEffect(() => {
    const socket = initSocket(driverToken);
    ...
    socket.on("new-accident", (data) => {
      console.log("🚨 New accident received:", data);
      alert("New accident nearby!");
    });
    ...
    <div className="bg-white rounded-lg shadow-md overflow-hidden">
      ...
      <div className="p-6">
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          ...
          <button
            onClick={() =>
              (window.location.href = `tel:${request.victimContact}`)
            }
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
          <p className="text-sm text-yellow-800">
            <strong>Note:</strong> Your live location is being shared with the
            control center and victim.
          </p>
        </div>
      </div>
    </div>
...
export default function AmbulanceDashboard() {
  ...
  useEffect(() => {
    if (ambulance.status !== "ONLINE") return;

    // Example: zoneId can be district / ward / radius bucket
    socket.emit("join-ambulance-zone", {
      zoneId: ambulance._id, // or ambulance.zoneId if available
    });
    return () => {
      socket.emit("leave-ambulance-zone", {
        zoneId: ambulance._id,
      });
    };
  }, [ambulance.status]);
  ...
  const handleCompleteCase = () => {
    setActiveCase(null);
    alert(
      "Case completed successfully! You are now available for new requests.",
    );
